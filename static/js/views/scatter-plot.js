App.View.ScatterPlot = React.createClass({
  ...
  handleSliderValueChanged: function(e) {
    if (e.detail.attributeId === 'Opacity') {
      ...
      this.setState({ numNeighbors: Math.round(e.detail.value * 100) });
    }
  },
  updateGlContext: function() {
    ...
    for (var src = 0; src < points.length; src++) {
      for (var k = 0; k < this.state.numNeighbors; k++) {
        ...
        this.edges.geometry.vertices.push(new THREE.Vector3(p.x * w, p.y * h, 0));
        this.edges.geometry.vertices.push(new THREE.Vector3(q.x * w, q.y * h, 0));
  ...
  update: function() {
    if (this.state.edgeGeometry !== undefined) {
