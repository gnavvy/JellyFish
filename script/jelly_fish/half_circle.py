...
def correlationM(data):
	...
	num1=len(data[0])
	for i in range(num0):
		avag=map(lambda x,y: x+y, data[i], avag if not avag==[0] else avag*len(data[0])) 
	...
	for i in range(num1-1):
		for j in range(i+1,num1):
			corrM[i,j]=sum((data[:,i]-avag[i])*(data[:,j]-avag[j]))/np.sqrt(sum((data[:,i]-avag[i])**2)*sum((data[:,j]-avag[j])**2))
...
def angle(data,percentage=10):
	...
	for i in subset[1:]:
		...
		index=sortCorr(corrM)   ## The index will be used to sort the data matrix. For future convenience, we should try out best not to use sorted correlation matrix
